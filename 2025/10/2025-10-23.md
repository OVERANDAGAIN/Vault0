...
非常好，这组图可以作为 **实验章节中 MOA 模块（对手建模模块）评估结果** 的展示页。下面是你可以直接放在 PPT 或论文解说中的英文文案（并附上中文讲解稿），逻辑清晰、篇幅适中，能自然过渡到 world model 与 reward 的后续图。

---

## ✅ **Slide Title: MOA Module Evaluation — Action Prediction Accuracy**

### **PPT 文案（英文）**

* The MOA (Model of Agents) module is trained to predict opponents’ next actions from trajectory data.
* The confusion matrix shows overall classification performance across seven action categories.
* Most predictions concentrate on the main cooperative and individual actions (classes 1–4), indicating that the model captures consistent behavioral patterns.
* The recall distribution reveals that the model performs particularly well on dominant and frequently occurring actions, while rare or ambiguous behaviors remain harder to identify.

*(Equation)*
$$
\text{Recall}_i = \frac{\text{True Positives}_i}{\text{All Samples with True Label } i}
$$
...
> “这里展示的是对手建模模块（MOA）的动作预测效果。
> 我们让模型根据历史轨迹去预测对手在下一时刻的动作类别。
> 上方的混淆矩阵展示了在七类动作上的整体分类表现，可以看到主流的行为类别（例如合作与个体动作，对应类别1到4）预测最为集中，说明模型已经学会捕捉到这些稳定的行为规律。
> 下方的柱状图展示了各类别的召回率（Recall），可以看出模型在样本量较大的动作类别上表现更好，而在一些稀有或模糊的行为类别上识别率相对较低。
> 总体来看，MOA 模块已经具备了较强的行为模式建模能力，为后续的目标推断与规划提供了可靠输入。”

---

### 💡 **可以加的视觉小优化建议**

1. 在混淆矩阵上方加简短标题：“Confusion Matrix — Action Classification (MOA)”。
2. 在 Recall 图下方加一句简短说明：“Model shows higher recall on frequent cooperative actions.”
3. 在页脚处添加引用：[Foerster et al., 2018; Aghajohari et al., 2023] 以呼应前面介绍的对手建模方法。

---

是否希望我接着帮你写下一页——**World Model Prediction Visualization** 的文案？（那一页通常介绍时序预测的准确性和未来状态建模结果，可以自然衔接到 reward 曲线部分。）
