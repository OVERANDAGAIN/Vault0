...
### **1. 固定对手策略假设（Fixed Policies）**

* 当前方法仅针对**固定策略的对手**进行建模，即假设对手在整个 episode 中使用不变的策略。
* 然而，现实中对手可能会在游戏过程中**动态改变或进化其策略（policy alterations / evolving policies）**。
* 应对**时变策略（changing policy opponents）**的离线对手建模仍是一个关键但尚未解决的难题。
  → *未来方向：开发能够捕捉和适应对手策略动态变化的 OOM 方法。*

---

### **2. 缺乏递归推理（Recursive Reasoning）**

* 当前模型只考虑单向的 opponent modeling，即智能体建模对手，但不考虑对手也在建模自己。
* 在复杂博弈中，**双方同时进行对手建模与反建模（recursive reasoning）** 是常见情形。
* 如何在**离线数据集上实现递归推理学习**（如基于信念建模或层级贝叶斯表示）是未来研究挑战。
  → *未来方向：在离线框架下引入递归推理层次（Theory of Mind-like modeling）。*

---

### **3. 单对手建模限制（Single Opponent Setting）**

* 实验中仅涉及**一个对手智能体**。
* 当存在多个对手（multi-opponent）时，必须考虑**不同对手之间的相关性与交互结构**，否则无法捕捉复杂博弈动力学。
  → *未来方向：扩展到多对手建模，研究跨对手的关系建模与联合表示。*

---

### **4. 仅限竞争场景（Competitive Settings）**

* 本文实验仅在**竞争性（competitive）** 环境中进行。
* 在**合作（cooperative）** 或 **混合（mixed-motive）** 场景中，对手建模与策略响应机制会面临不同的挑战（例如共享目标、信任建立、合作破裂等）。
  → *未来方向：将 OOM 扩展至合作型和混合型多智能体任务。*
