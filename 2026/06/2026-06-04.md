...
它表示当前 episode 中的广告曝光历史、最近广告负担和用户对广告的可观察反馈。

定义：

$$E_t = (n_t^{\text{ad}}, l_t^{\text{ad}}, g_t^{\text{ad}},f_t^{\text{ad}})$$

其中：

| 符号                | 名称                  | 含义                              |
| ----------------- | ------------------- | ------------------------------- |
| $n_t^{\text{ad}}$ | num_ads_shown       | 当前 episode 中已经展示的广告数量           |
| $l_t^{\text{ad}}$ | recent_ad_load      | 最近 $K$ 个 decision step 中的广告展示次数 |
| $g_t^{\text{ad}}$ | ad_gap              | 距离上一次广告展示的 decision step 数      |
| $f_t^{\text{ad}}$ | ad_feedback_summary | 用户对广告的历史反馈摘要，基于可观察文本和行为         |
...
```json
{
  ...
  "recent_ad_load": 1,
  "ad_gap": 2,
  "last_ad_outcome": "ignored",
  "ad_feedback_summary": "用户没有明确拒绝广告，但对上一条广告没有追问。"
}
```
