...
- [?] 
- [x] 
1. 先训练opponent:  python main.py --config=qmix  --env-config=sc2 with env_args.map_name=8m
2. 再加载 `agent.th` 进行预训练 python main.py --config=qmix  --env-config=sc2 with env_args.map_name=8m --mac=basic_mac --am_model=omg_am --learner=am_learner --obs_is_state=True --init_param_path=SMAC/sacred/models/qmix__2025-02-11_13-42-21/33
...
3. `args.am_model` 只会在 `am_leaner` 中使用
4. `alg_args.am_model` 在 `om_controller` 中使用

`loss_func` 
5. 只 `base_am` 和 `omg_am` 中被定义
6. 只在 `am_learner` 中被调用
7.  `omg_learner` 中使用的是 `omg_loss_func` 
...
8. 在 `learners` 中被调用: `mac.load_model()`
9. 在 `am` 中被定义：`load_state_dict` 
	1. `vae(omg_am)`
	2. `base_am`
10. 在 `controller` 中定义（即1中的调用）：
	1. `agent - agent.th`
	2. `am load...()`
