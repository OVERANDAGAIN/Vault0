...
1. 先训练opponent:  
 ```bash
python main.py --config=qmix  --env-config=sc2 with env_args.map_name=8m
```
   
2. 再加载 `agent.th` 进行预训练 
```bash
   python main.py --config=qmix  --env-config=sc2 with env_args.map_name=8m --mac=basic_mac --am_model=omg_am --learner=am_learner --obs_is_state=True --init_param_path=SMAC/sacred/models/qmix__2025-02-11_13-42-21/33
   ```
3.  训练CVAE: 
```bash
