...
$$\Delta_t^{\text{topic}}\in[0,1]$$

话题偏移项刻画当前回复是否破坏 topic flow。

设当前 topic segment 表示为：

$$z_t=Z(\sigma_t,\ell_t^K)$$

其中 $Z(\cdot)$ 可以由 Monitor 或 embedding model 得到。

如果 Planner 本轮没有选择换话题：
...
在我们的主 reward 公式中不显式把 $y_t$ 作为输入，是因为 $y_t$ 已经出现在 $s_{t+1}$ 的最近对话上下文 $\ell_{t+1}^{K}$ 中。实现时可以从 $s_{t+1}$ 中取出本轮系统回复进行计算。
