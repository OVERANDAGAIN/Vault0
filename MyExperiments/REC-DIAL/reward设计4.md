...
# 二、从“内容流”到“对话轨迹”：LLM 多轮对话的抽象

在传统流式推荐中，用户消费的是一个由 item 组成的内容流。每个 item 可以是视频、新闻、商品或广告。

在 LLM 多轮对话中，用户消费的对象不再是静态 item，而是随着交互不断演化的语义轨迹。REC-DIAL 可以将其抽象为一个多轮 decision process。
...
其中，每个 decision step 以系统的一次 Planner action 为中心。系统先观察当前用户输入和历史，构造状态 $s_t$，再选择动作 $a_t$，生成系统回复 $y_t$，随后 Mock User 返回下一轮用户输入 $q_{t+1}$。

虽然对话中会出现局部连续的话题或子任务，但 REC-DIAL 不显式定义 topic segment。当前话题上下文由 Planner state 中的 BeliefState 和 DialogueContext 共同表示：

$$\mathrm{TopicContext}_t = (B_t, D_t)$$

其中，$B_t$ 表示当前用户目标、偏好、约束和未解决问题，$D_t$ 表示长期 dialogue memory 和最近局部上下文。

因此，reward 仍然在 decision-step level 计算，但其中部分 reward 项需要利用当前 topic context 判断回复是否连贯、广告是否打断当前任务、topic switch 是否自然。

| 层级 | 定义 | 作用 |
| --- | --- | --- |
| decision step | 一次 system-side policy decision 形成的状态转移 | 提供 step-level reward |
| topic context | 由 $B_t$ 和 $D_t$ 表示的当前语义上下文 | 提供话题连贯性和 topic drift 判断依据 |
...
# 四、decision step、topic context 与 reward 的关系

广告插入发生在某一个 decision step，但其影响可能跨越多个后续 step。

例如：

```text
Step 3: 系统插入一个表面相关的广告
Step 4: 用户没有立即反感，继续提问
Step 5: 用户回复变短
Step 6: 用户提前结束对话
```

如果只看 Step 3，reward 可能是正的，因为广告展示带来了商业价值，且用户没有直接拒绝。

但从 episode 角度看，这次广告可能破坏了任务主线，使用户逐渐失去兴趣，最终提前退出。

因此：

> 单步即时反馈不足以反映广告的长期副作用。Reward 需要同时包含 step-level 局部信号、topic-context-level 语义约束和 episode-level 终局结果。
...
## 4.2 Topic-context-level reference

REC-DIAL 不显式维护 topic segment，而是使用当前 Planner state 中的 topic context 作为语义参照：
...
```text
“推荐写论文工具”在“论文写作”上下文中是合理的；
但在“旅游住宿规划”上下文中就是明显偏移。
```
...
因此，最终优化目标不是单步 reward，而是累计回报：

$$G = \sum_{t=0}^{T-1} \gamma^t r_t + r_{\mathrm{terminal}}$$
...
| 层级 | reward 作用 |
| --- | --- |
| decision step | 提供局部反馈 |
| topic context | 提供语义约束 |
| episode | 定义长期目标 |
...
用户反馈 $q_{t+1}$ 本身不是 reward，而是 raw feedback。它进入更新后的历史 $H_{t+1}$，再通过 Monitor 反映到下一状态 $s_{t+1}$，并由 RewardCalculator 解释为当前 step 的标量 reward。

---

## 6.1 Monitor 与 RewardCalculator 的边界

Monitor 和 RewardCalculator 会使用同一批底层信号，例如用户输入、广告事件、话题变化和历史统计。但二者职责不同。
...
对话持续性项刻画用户是否愿意继续交互。它是长期留存的 step-level proxy。

它主要根据 $s_{t+1}$ 中的用户反馈和 $\mathrm{done}_t$ 状态判断：

| 情况 | $\kappa_t^{\mathrm{cont}}$ |
| --- | --- |
| 用户继续提出相关问题 | 高 |
| 用户给出有效补充信息 | 高 |
| 用户只给出简短但未退出的反馈 | 中 |
...
=
\begin{cases}
+R_{\mathrm{success}}, & \text{任务完成} \\
-R_{\mathrm{annoy}}, & \text{因广告反感退出} \\
-R_{\mathrm{fail}}, & \text{任务未完成退出} \\
0, & \text{中性结束}
\end{cases}$$
...
如果直接累加每步 reward，Planner 可能学到一种错误策略：
...
设定最大 decision step 数：

$$T \leq T_{\max}$$

超过最大 step 数后 episode 强制结束。
...
这样可以避免把“用户完成任务后结束”错误惩罚，也避免 Planner 通过无限延长对话获得高回报。