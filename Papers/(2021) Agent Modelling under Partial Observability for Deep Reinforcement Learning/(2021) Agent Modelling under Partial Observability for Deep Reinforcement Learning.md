...
对手/队友策略集合是固定的，而且每个 episode 内固定不变。

LIAM 的有效性依赖于一个前提：从 ego 的局部交互历史中，确实能辨识出其他 agent 的行为；因此它更适用于固定策略族、反应性强、低维观测、训练时可访问他人轨迹的 CTDE 场景，而不适用于强非平稳、弱耦合或高维视觉主导的开放环境。
# Focus
