...
1. **`execution_plan`**
   - 核心数据流入口，定义了数据采样、回放和训练的流程。

2. **`alpha_zero_loss`**
   - 用于计算策略网络和价值网络的损失，是训练过程的关键函数。

3. **`AlphaZeroPolicyWrapperClass`**
   - 封装了策略和模型，调用模型进行前向传播，并结合`alpha_zero_loss`进行优化。

4. **`AlphaZeroTrainer`**
   - 整合了所有配置和训练逻辑，是任务的总体管理器。

5. **`DEFAULT_CONFIG`**
   - 配置了训练和 MCTS 的参数，为数据流和优化过程提供必要的超参数支持。
